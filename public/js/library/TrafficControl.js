...
(function() {
    ...
    g.prototype._popUpDiv = function(F, w) {
            ...
            F.timer = setInterval(function() {
                e(I + (new Date()).getTime(), function() {
                    if (F.bShow) {
                        var N = g.curTime;
                        var M = N.getHours();
                        var O = (M < 10 ? ("0" + M) : M) + ":" + (N.getMinutes() < 10 ? ("0" + N.getMinutes()) : N.getMinutes());
                        y.innerHTML = O;
                        F.hideTraffic();
                        F.showTraffic()
                    }
        ...
        function u() {
            ...
            I.push("</div></div></div>");
            I.push('<div class="maplibTcClear" style="text-align: center; color: #ccc;">（基于历史流量统计预测 仅供参考）</div>');
            I.push("</div></div></div>");
            return I.join("")
        }
        function C(J) {
        ...
        if (j.addEventListener) {
            ...
            if (j.attachEvent) {
                j.attachEvent("onreadystatechange", function(n) {
                    ...
                    if (m && (m.readyState == "loaded" || m.readyState == "complete")) {
                        m.parentNode.removeChild(m);
                        if (l) {
                            l()
                        }
